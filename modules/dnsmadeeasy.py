...
class DME2(object):
    ...
    def query(self, resource, method, data=None):
        ...
        if info['status'] not in (200, 201, 204):
            ...
            self.module.fail_json(msg="Fail: %s %s %s" % (url,"Status: " + str(info['msg']) + ", Message: " + str(info['body']),data))
        try:
        ...
        if record_type in ["CNAME", "ANAME", "HTTPRED", "PTR"]:
            ...
            for result in self.all_records:
                if record_type == "MX":
                    ...
                    value = '"'+record_value+'"'
                    if record_name == '' and result['name'] == '' and result['type'] == record_type and result['value'] == value:
                      return result;
                    elif record_name != '' and result['name'] == record_name and result['type'] == record_type:
                      return result;
                else:
                    value = record_value
...
def main():
    ...
    module = AnsibleModule(
        argument_spec=dict(
            ...
            record_ttl=dict(required=False, type='int'),
    ...
    if state == 'present':
        ...
        if not current_record:
            ...
            if ( record_type in ["MX","TXT"] ):
              module.exit_json(changed=True, result=dict(record=record, monitor=False))
            else:
